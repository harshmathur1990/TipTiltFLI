...
//        cout.flush();
        ...
        if (((MODE == MOTOR) && (xxis == 0)) || (MODE == ACTUATOR)) {
            ...
            xlog("X-Calibration Log");
            xlog("Number of frames are " + std::to_string(NFRAMES));
            xlog("Number of frames per calib position are " + std::to_string(NUM_FRAME_PER_CALIB_POS));
            Err = doCalib(NFRAMES, NUM_FRAME_PER_CALIB_POS, 0, MODE);
        }

        if (((MODE == MOTOR) && (xxis == 1)) || (MODE == ACTUATOR)) {
            cout << "Starting Y Calibration... " << endl;
            ylog("Y-Calibration Log");
            ylog("Number of frames are " + std::to_string(NFRAMES));
            ylog("Number of frames per calib position are " + std::to_string(NUM_FRAME_PER_CALIB_POS));
...
int doCalib(int NFRAMES, int NUM_FRAME_PER_CALIB_POS, int AXIS, int MODE){
    ...
    std::function<int(std::string)> axisLog;
    std::function<int(std::string)> shiftLog;
    if (AXIS==0) {
        Axis = "X";
        axisLog = xlog;
        shiftLog = xvoltagefilelog;
        motor = 2;
    }
    else {
        Axis = "Y";
        axisLog = ylog;
        shiftLog = yvoltagefilelog;
        motor = 1;
    }
    ...
    std::snprintf(logLine, sizeof(logLine), "Image acquired : %s", fname.c_str());
    axisLog(logLine);
...
//        XYIND = getImageShift(binnedImage);
        std::snprintf(logLine, sizeof(logLine), "%s%lf,%lf", tempLogString, shift_x / NUM_FRAME_PER_CALIB_POS, shift_y / NUM_FRAME_PER_CALIB_POS);
        shiftLog(logLine);
    ...
    std::snprintf(logLine, sizeof(logLine), "Number of frames: %d, In seconds: %.6f", NFRAMES, dt.count());
    axisLog(logLine);
    std::snprintf(logLine, sizeof(logLine), "Frame rate: %.6f", NFRAMES / dt.count());
    axisLog(logLine);
    Err = closeDAQ();
    if (MODE == MOTOR) {
        exitMotor(1);
